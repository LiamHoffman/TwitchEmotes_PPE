PPETwitchEmotes = {
	["Path"] = "Interface\\AddOns\\TwitchEmotes_PPE\\emotes"
};
...
function PPETwitchEmotes:init()
	...
	TwitchEmotes:AddCategory("Peeper's Palace", emoteIDs);
end



function PPE_startup()
    local suggestions = {};
    local i = 1;
    PPETwitchEmotes:init();
    for k, v in pairs(PPE_emotes) do
    ...
    PPE_initsuggestions(suggestions);
end

function PPE_concat(t1, t2)
    for i=1,#t2 do
        t1[#t1+1] = t2[i]
    end
    return t1
...
function PPE_initsuggestions(suggestions)
    if AllTwitchEmoteNames ~= nil and Emoticons_Settings ~= nil and Emoticons_RenderSuggestionFN ~= nil and Emoticons_Settings["ENABLE_AUTOCOMPLETE"] then
        ...
        PPE_concat(suggestions, AllTwitchEmoteNames);
